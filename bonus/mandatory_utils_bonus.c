...
/*   Updated: 2024/12/17 17:24:08 by francema         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	lputunsigned(unsigned int n, int *p_b)
{
	...
	s = malloc(sizeof(char) * (ft_uns_len(n, 10) + 1));
	if (!s)
		return ;
	build_num(n, "0123456789", s);
	lputstr(s, p_b);
	free(s);
}
...
void	lputexa(t_info *info, char c)
{
	...
	if (c == 'X')
		...
		lputstr("0x", &(info->p_b));
	buf = malloc(sizeof(char) * (ft_uns_len(info->u_tmp, 16) + 1));
	if (!buf)
		return ;
...
void	lputadrr(t_info *info)
{
	...
	info->u_tmp = 0;
	buf = malloc(sizeof(char) * (ft_uns_len(info->p_tmp, 16) + 1));
	if (!buf)
		return ;
	lputstr("0x", &(info->p_b));
